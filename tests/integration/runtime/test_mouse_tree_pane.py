...
class AppRuntimeMouseTestsPart1(unittest.TestCase):
    ...
    def test_single_click_file_switch_records_history_back_and_forward(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp).resolve()
            ...
            self.assertEqual(snapshots["current_before_back"], second_file.resolve())
            self.assertTrue(bool(snapshots["back_moved"]))
            self.assertEqual(snapshots["current_after_back"], first_file.resolve())
            self.assertTrue(bool(snapshots["forward_moved"]))
            self.assertEqual(snapshots["current_after_forward"], second_file.resolve())
