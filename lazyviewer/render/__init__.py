...
# Keep in sync with git_status._REMOVED_BG_SGR for diff previews.
...
def _iter_diff_logical_line_ranges(
    ...
    while idx < len(text_lines):
        start_idx = idx
        if wrap_text:
            while idx < len(text_lines) - 1 and not _line_has_newline_terminator(text_lines[idx]):
                idx += 1
...
def _diff_preview_uses_plain_markers(
    ...
    logical_ranges = _iter_diff_logical_line_ranges(text_lines, wrap_text)
...
def _diff_source_line_for_display_index(
    ...
    source_line = 1
    use_plain_markers = _diff_preview_uses_plain_markers(text_lines, wrap_text)
    for start_idx, end_idx in _iter_diff_logical_line_ranges(text_lines, wrap_text):
        is_removed = _diff_preview_logical_line_is_removed(
...
def _source_line_display_index(
    ...
    if preview_is_git_diff:
        source_idx = 1
        use_plain_markers = _diff_preview_uses_plain_markers(text_lines, wrap_text)
        for start_idx, _end_idx in _iter_diff_logical_line_ranges(text_lines, wrap_text):
            is_removed = _diff_preview_logical_line_is_removed(
                text_lines[start_idx],
                use_plain_markers=use_plain_markers,
            )
...
def _source_line_raw_text(
    ...
    start_idx = _source_line_display_index(
        text_lines,
        source_line,
        wrap_text,
...
def _source_line_is_blank(
    ...
    source_text = _source_line_raw_text(
        text_lines,
        source_line,
        wrap_text,
        preview_is_git_diff=preview_is_git_diff,
...
def _source_line_count(
    ...
    if preview_is_git_diff:
        ...
        source_count = 0
        for start_idx, _end_idx in _iter_diff_logical_line_ranges(text_lines, wrap_text):
            if _diff_preview_logical_line_is_removed(
                text_lines[start_idx],
                use_plain_markers=use_plain_markers,
            ):
                continue
...
def _next_nonblank_source_line(
    ...
    total_lines = _source_line_count(
        text_lines,
        wrap_text,
        preview_is_git_diff=preview_is_git_diff,
    )
    for source_line in range(max(1, start_line), total_lines + 1):
        if not _source_line_is_blank(
            text_lines,
            source_line,
            wrap_text,
            preview_is_git_diff=preview_is_git_diff,
...
def _blank_line_exits_symbol_scope(
    ...
    next_nonblank = _next_nonblank_source_line(
        text_lines,
        source_line + 1,
        wrap_text,
        preview_is_git_diff=preview_is_git_diff,
...
def sticky_symbol_headers_for_position(
    *,
    ...
    if preview_is_git_diff:
        start_source = _diff_source_line_for_display_index(text_lines, text_start, wrap_text)
    else:
