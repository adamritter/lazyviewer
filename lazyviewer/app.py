...
def run_pager(content: str, path: Path, style: str, no_color: bool, nopager: bool) -> None:
    ...
    def handle_picker_key(key: str) -> tuple[bool, bool]:
        if not state.picker_active:
            return False, False
    ...
    def handle_tree_filter_key(key: str) -> bool:
    ...
    with terminal.raw_mode():
        while True:
            ...
            picker_handled, picker_should_quit = handle_picker_key(key)
            if picker_should_quit:
                break
            if picker_handled:
                continue
            if handle_tree_filter_key(key):
                continue
