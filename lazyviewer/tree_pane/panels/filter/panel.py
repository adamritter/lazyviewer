...
class FilterPanel:
    ...
    def toggle_mode(self, mode: str) -> None:
        ...
        if self.owner.state.filter.active:
            if self.owner.state.filter.mode == mode and self.owner.state.filter.editing:
                self.owner.close_tree_filter(clear_query=True)
            elif self.owner.state.filter.mode != mode:
                self.owner.open_tree_filter(mode)
            else:
                self.owner.state.filter.editing = True
    ...
    def open(self, mode: str = "files") -> None:
        ...
        was_active = self.owner.state.filter.active
        previous_mode = self.owner.state.filter.mode
        if not self.owner.state.filter.active:
            self.owner.state.filter.previous_browser_visible = self.owner.state.layout.browser_visible
        ...
        self.owner.state.filter.active = True
        self.owner.set_tree_filter_prompt_row_visible(True)
        self.owner.state.filter.mode = mode
        self.owner.state.filter.editing = True
        self.owner.state.filter.origin = self.owner.current_jump_location() if mode == "content" else None
        self.owner.state.filter.query = ""
        self.owner.state.filter.match_count = 0
        self.owner.state.filter.truncated = False
        self.owner.reset_tree_filter_session_state()
        if was_active and previous_mode != mode:
            self.owner.rebuild_tree_entries(preferred_path=self.owner.state.workspace.current_path.resolve())
        self.owner.state.interface.dirty = True
    ...
    def close(
        ...
        previous_browser_visible = self.owner.state.filter.previous_browser_visible
        restore_location: JumpLocation | None = None
        if (
            restore_origin
            and self.owner.state.filter.mode == "content"
            and self.owner.state.filter.origin is not None
        ):
            restore_location = self.owner.state.filter.origin.normalized()
        self.owner.state.filter.active = False
        self.owner.state.filter.editing = False
        self.owner.set_tree_filter_prompt_row_visible(True)
        self.owner.state.filter.mode = "files"
        if clear_query:
            self.owner.state.filter.query = ""
            self.owner.state.filter.truncated = False
        self.owner.reset_tree_filter_session_state()
        self.owner.state.filter.previous_browser_visible = None
        ...
        if restore_location is not None:
            ...
            self.owner.state.preview.max_scroll = max(0, len(self.owner.state.preview.lines) - self.owner.visible_content_rows())
            self.owner.state.preview.scroll = max(0, min(restore_location.start, self.owner.state.preview.max_scroll))
            self.owner.state.preview.horizontal_scroll = 0 if self.owner.state.preview.wrap else max(0, restore_location.text_x)
        else:
            self.owner.rebuild_tree_entries(preferred_path=self.owner.state.workspace.current_path.resolve())
        self.owner.state.filter.origin = None
        self.owner.state.interface.dirty = True
        if self.owner.on_tree_filter_state_change is not None:
            self.owner.on_tree_filter_state_change()
    ...
    def activate_selection(self) -> None:
        ...
        if not self.owner.state.workspace.entries:
            if self.owner.state.filter.mode == "content":
                self.owner.state.filter.editing = False
                self.owner.state.interface.dirty = True
            else:
                self.owner.close_tree_filter(clear_query=True)
        ...
        if entry.is_dir:
            candidate_idx = self.owner.nearest_tree_filter_result_entry_index(self.owner.state.workspace.selected)
            if candidate_idx is None:
                self.owner.close_tree_filter(clear_query=True)
                return
            self.owner.state.workspace.selected = candidate_idx
            entry = self.owner.state.workspace.entries[self.owner.state.workspace.selected]
    ...
    def handle_key(
        ...
        *,
        ...
        def apply_live_filter_query(query: str) -> None:
            content_mode = self.owner.state.filter.mode == "content"
        ...
        if self.owner.state.filter.editing:
            ...
            if key == "ESC":
                self.owner.close_tree_filter(
                    clear_query=True,
                    restore_origin=self.owner.state.filter.mode == "content",
                )
                return True
            if key == "ENTER":
            ...
            if key == "DOWN" or key == "CTRL_J":
                ...
                return True
            if key == "BACKSPACE":
                if self.owner.state.filter.query:
                    apply_live_filter_query(self.owner.state.filter.query[:-1])
                return True
            if key == "CTRL_U":
                if self.owner.state.filter.query:
            ...
            if len(key) == 1 and key.isprintable():
                apply_live_filter_query(self.owner.state.filter.query + key)
                return True
            return True
