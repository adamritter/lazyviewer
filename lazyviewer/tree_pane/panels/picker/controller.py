"""Picker and navigation controller with explicit methods (no mixins)."""

from __future__ import annotations

import shutil
import time
from collections.abc import Callable
from pathlib import Path

from ....runtime.config import save_named_marks, save_show_hidden
from ....session.navigation import JumpLocation, is_named_mark_key
from ....session import SessionState
from ....ports import (
    PreviewSelectedEntry,
    RebuildScreenLines,
    RebuildTreeEntries,
    RefreshGitStatus,
    RefreshPreview,
)
from ....search.fuzzy import fuzzy_match_labels
from ...workspace_roots import normalized_workspace_expanded_sections
from .line_map import first_display_index_for_source_line, source_line_for_display_index
from .panel import PickerPanel

PICKER_RESULT_LIMIT = 200


class NavigationController:
    """State-bound picker/navigation operations used by key handlers."""

    def __init__(
        self,
        *,
        state: SessionState,
        command_palette_items: tuple[tuple[str, str], ...],
        rebuild_screen_lines: RebuildScreenLines,
        rebuild_tree_entries: RebuildTreeEntries,
        preview_selected_entry: PreviewSelectedEntry,
        schedule_tree_filter_index_warmup: Callable[[], None],
        mark_tree_watch_dirty: Callable[[], None],
        reset_git_watch_context: Callable[[], None],
        refresh_git_status_overlay: RefreshGitStatus,
        visible_content_rows: Callable[[], int],
        refresh_rendered_for_current_path: RefreshPreview,
        open_tree_filter: Callable[[str], None],
    ) -> None:
        """Bind controller methods from explicit runtime hooks."""

        self.state = state
        self.command_palette_items = command_palette_items
        self.rebuild_screen_lines = rebuild_screen_lines
        self.rebuild_tree_entries = rebuild_tree_entries
        self.preview_selected_entry = preview_selected_entry
        self.schedule_tree_filter_index_warmup = schedule_tree_filter_index_warmup
        self.mark_tree_watch_dirty = mark_tree_watch_dirty
        self.reset_git_watch_context = reset_git_watch_context
        self.refresh_git_status_overlay = refresh_git_status_overlay
        self.visible_content_rows = visible_content_rows
        self.refresh_rendered_for_current_path = refresh_rendered_for_current_path
        self.open_tree_filter = open_tree_filter
        self.panel = PickerPanel(self)

    # matching
    def refresh_symbol_picker_matches(self, reset_selection: bool = False) -> None:
        """Recompute visible symbol matches from current picker query."""
        matched = fuzzy_match_labels(
            self.state.picker.query,
            self.state.picker.symbol_labels,
            limit=PICKER_RESULT_LIMIT,
        )
        self.state.picker.matches = []
        self.state.picker.match_labels = [label for _, label, _ in matched]
        self.state.picker.match_lines = [self.state.picker.symbol_lines[idx] for idx, _, _ in matched]
        self.state.picker.match_commands = []
        if self.state.picker.match_labels:
            self.state.picker.message = ""
        elif not self.state.picker.message:
            self.state.picker.message = " no matching symbols"
        self.state.picker.selected = 0 if reset_selection else max(
            0,
            min(self.state.picker.selected, max(0, len(self.state.picker.match_labels) - 1)),
        )
        if reset_selection or not self.state.picker.match_labels:
            self.state.picker.list_start = 0

    def refresh_command_picker_matches(self, reset_selection: bool = False) -> None:
        """Recompute command palette matches from current picker query."""
        matched = fuzzy_match_labels(
            self.state.picker.query,
            self.state.picker.command_labels,
            limit=PICKER_RESULT_LIMIT,
        )
        self.state.picker.matches = []
        self.state.picker.match_labels = [label for _, label, _ in matched]
        self.state.picker.match_lines = []
        self.state.picker.match_commands = [self.state.picker.command_ids[idx] for idx, _, _ in matched]
        if self.state.picker.match_labels:
            self.state.picker.message = ""
        elif not self.state.picker.message:
            self.state.picker.message = " no matching commands"
        self.state.picker.selected = 0 if reset_selection else max(
            0,
            min(self.state.picker.selected, max(0, len(self.state.picker.match_labels) - 1)),
        )
        if reset_selection or not self.state.picker.match_labels:
            self.state.picker.list_start = 0

    def refresh_active_picker_matches(self, reset_selection: bool = False) -> None:
        """Refresh matches for whichever picker mode is currently active."""
        if self.state.picker.mode == "commands":
            self.refresh_command_picker_matches(reset_selection=reset_selection)
            return
        self.refresh_symbol_picker_matches(reset_selection=reset_selection)

    # picker key handling
    def handle_picker_key(self, key: str, double_click_seconds: float) -> tuple[bool, bool]:
        """Handle one key while picker is active."""
        return self.panel.handle_key(key, double_click_seconds)

    # history/navigation
    def current_jump_location(self) -> JumpLocation:
        """Capture current file path and scroll offsets as a jump location."""
        return JumpLocation(
            path=self.state.workspace.current_path.resolve(),
            start=max(0, self.state.preview.scroll),
            text_x=max(0, self.state.preview.horizontal_scroll),
        )

    def record_jump_if_changed(self, origin: JumpLocation) -> None:
        """Record ``origin`` in jump history only when position actually changed."""
        normalized_origin = origin.normalized()
        if self.current_jump_location() == normalized_origin:
            return
        self.state.navigation.history.record(normalized_origin)

    def apply_jump_location(self, location: JumpLocation) -> bool:
        """Apply jump location and clamp offsets to current rendered content."""
        target = location.normalized()
        current_path = self.state.workspace.current_path.resolve()
        path_changed = target.path != current_path
        if path_changed:
            self.jump_to_path(target.path)

        self.state.preview.max_scroll = max(0, len(self.state.preview.lines) - self.visible_content_rows())
        clamped_start = max(0, min(target.start, self.state.preview.max_scroll))
        clamped_text_x = 0 if self.state.preview.wrap else max(0, target.text_x)
        prev_start = self.state.preview.scroll
        prev_text_x = self.state.preview.horizontal_scroll
        self.state.preview.scroll = clamped_start
        self.state.preview.horizontal_scroll = clamped_text_x
        return path_changed or self.state.preview.scroll != prev_start or self.state.preview.horizontal_scroll != prev_text_x

    def jump_back_in_history(self) -> bool:
        """Jump to previous history location, returning whether state changed."""
        target = self.state.navigation.history.go_back(self.current_jump_location())
        if target is None:
            return False
        return self.apply_jump_location(target)

    def jump_forward_in_history(self) -> bool:
        """Jump to next history location, returning whether state changed."""
        target = self.state.navigation.history.go_forward(self.current_jump_location())
        if target is None:
            return False
        return self.apply_jump_location(target)

    def set_named_mark(self, mark_key: str) -> bool:
        """Store current jump location under a valid named-mark key."""
        if not is_named_mark_key(mark_key):
            return False
        self.state.navigation.marks[mark_key] = self.current_jump_location()
        save_named_marks(self.state.navigation.marks)
        return True

    def jump_to_named_mark(self, mark_key: str) -> bool:
        """Jump to saved named mark and push current location onto history."""
        if not is_named_mark_key(mark_key):
            return False
        target = self.state.navigation.marks.get(mark_key)
        if target is None:
            return False
        origin = self.current_jump_location()
        if target.normalized() == origin:
            return False
        self.state.navigation.history.record(origin)
        return self.apply_jump_location(target)

    def _normalize_tree_roots(self, *, include_active: bool = True) -> list[Path]:
        """Normalize roots and synchronize per-section expansion state."""
        roots = [raw_root.resolve() for raw_root in self.state.workspace.roots]
        active_root = self.state.workspace.active_root.resolve()
        if include_active and not any(root == active_root for root in roots):
            roots.append(active_root)
        normalized_roots, sections, flat_union = normalized_workspace_expanded_sections(
            roots,
            active_root,
            self.state.workspace.expanded_by_root,
            self.state.workspace.expanded,
            include_active=include_active,
        )
        self.state.workspace.roots = normalized_roots
        self.state.workspace.expanded_by_root = sections
        self.state.workspace.expanded = flat_union
        return normalized_roots

    def _switch_active_tree_root(
        self,
        target_root: Path,
        *,
        preferred_path: Path | None,
        preserve_old_root_expanded: bool = False,
        include_previous_root: bool = True,
    ) -> None:
        """Switch active root, rebuild view, and resync watch/git state."""
        old_root = self.state.workspace.active_root.resolve()
        target_root = target_root.resolve()
        roots = self._normalize_tree_roots(include_active=include_previous_root)
        if target_root not in roots:
            roots.append(target_root)
        self.state.workspace.roots = roots

        self.state.workspace.active_root = target_root
        if preserve_old_root_expanded and old_root != target_root:
            self.state.workspace.expanded = {target_root, old_root}
        else:
            self.state.workspace.expanded = {target_root}
        self.state.workspace.expanded_by_root = []
        self._normalize_tree_roots(include_active=include_previous_root)

        focused_path = target_root if preferred_path is None else preferred_path.resolve()
        if not focused_path.is_relative_to(target_root):
            focused_path = target_root
        self.rebuild_tree_entries(preferred_path=focused_path, center_selection=True)
        self.preview_selected_entry(force=True)
        self.schedule_tree_filter_index_warmup()
        self.mark_tree_watch_dirty()
        self.reset_git_watch_context()
        self.refresh_git_status_overlay(force=True)
        self.state.interface.dirty = True

    def _selected_target_and_root(self) -> tuple[Path, Path]:
        """Return selected target path and its directory root candidate."""
        selected_entry = (
            self.state.workspace.entries[self.state.workspace.selected]
            if self.state.workspace.entries and 0 <= self.state.workspace.selected < len(self.state.workspace.entries)
            else None
        )
        if selected_entry is not None:
            selected_target = selected_entry.path.resolve()
            target_root = selected_target if selected_entry.is_dir else selected_target.parent.resolve()
            return selected_target, target_root

        selected_target = self.state.workspace.current_path.resolve()
        target_root = selected_target if selected_target.is_dir() else selected_target.parent.resolve()
        return selected_target, target_root

    def _selected_workspace_section(self) -> int | None:
        """Return selected entry workspace section index when available."""
        roots = self._normalize_tree_roots()
        if not roots:
            return None
        if self.state.workspace.entries and 0 <= self.state.workspace.selected < len(self.state.workspace.entries):
            entry = self.state.workspace.entries[self.state.workspace.selected]
            if entry.workspace_section is not None and 0 <= entry.workspace_section < len(roots):
                return entry.workspace_section
            if entry.workspace_root is not None:
                resolved_root = entry.workspace_root.resolve()
                return next((idx for idx, root in enumerate(roots) if root == resolved_root), None)
        return None

    def reveal_path_in_tree(self, target: Path) -> None:
        """Expand ancestor directories and rebuild tree focused on ``target``."""
        target = target.resolve()
        roots = self._normalize_tree_roots()
        if not roots:
            return
        scope_section = next((idx for idx, root in enumerate(roots) if target.is_relative_to(root)), 0)
        scope_root = roots[scope_section]
        sections = list(self.state.workspace.expanded_by_root)
        while len(sections) < len(roots):
            sections.append(set())
        scoped = set(sections[scope_section])
        scoped.add(scope_root)
        if target != scope_root:
            parent = target.parent
            while True:
                resolved = parent.resolve()
                if resolved == scope_root:
                    break
                scoped.add(resolved)
                if resolved.parent == resolved:
                    break
                parent = resolved.parent
        sections[scope_section] = scoped
        self.state.workspace.expanded_by_root = sections
        self.state.workspace.expanded = set().union(*sections)
        self.rebuild_tree_entries(
            preferred_path=target,
            preferred_workspace_root=scope_root,
            preferred_workspace_section=scope_section,
            center_selection=True,
        )
        self.mark_tree_watch_dirty()

    def jump_to_path(self, target: Path) -> None:
        """Reveal and open ``target`` path in tree and source preview state."""
        target = target.resolve()
        current_root = self.state.workspace.active_root.resolve()
        if not target.is_relative_to(current_root):
            workspace_root = next(
                (root for root in self._normalize_tree_roots() if target.is_relative_to(root)),
                None,
            )
            if workspace_root is not None:
                self._switch_active_tree_root(
                    workspace_root,
                    preferred_path=target,
                )
        self.reveal_path_in_tree(target)
        self.state.workspace.current_path = target
        self.refresh_rendered_for_current_path()

    def jump_to_line(self, line_number: int) -> None:
        """Scroll source preview near ``line_number`` and reset horizontal offset."""
        visible_rows = max(1, self.visible_content_rows())
        self.state.preview.max_scroll = max(0, len(self.state.preview.lines) - visible_rows)
        max_line_index = max(0, len(self.state.preview.lines) - 1)
        anchor = max(0, min(line_number, max_line_index))
        centered = max(0, anchor - max(1, visible_rows // 3))
        self.state.preview.scroll = max(0, min(centered, self.state.preview.max_scroll))
        self.state.preview.horizontal_scroll = 0

    # view actions
    def current_term_columns(self) -> int:
        """Return current terminal width used for render rebuild calls."""
        return shutil.get_terminal_size((80, 24)).columns

    def reroot_to_parent(self) -> None:
        """Replace selected workspace root with its parent directory."""
        roots = self._normalize_tree_roots()
        if not roots:
            return
        selected_target = (
            self.state.workspace.entries[self.state.workspace.selected].path.resolve()
            if self.state.workspace.entries and 0 <= self.state.workspace.selected < len(self.state.workspace.entries)
            else self.state.workspace.current_path.resolve()
        )
        selected_section = self._selected_workspace_section()
        if selected_section is None:
            active_root = self.state.workspace.active_root.resolve()
            selected_section = next((idx for idx, root in enumerate(roots) if root == active_root), 0)
        selected_section = max(0, min(selected_section, len(roots) - 1))
        selected_scope = roots[selected_section]
        parent_root = selected_scope.parent.resolve()
        if parent_root == selected_scope:
            return

        normalized_roots = list(roots)
        normalized_roots[selected_section] = parent_root
        self.state.workspace.roots = normalized_roots

        old_sections = list(self.state.workspace.expanded_by_root)
        while len(old_sections) < len(roots):
            old_sections.append(set())
        new_sections: list[set[Path]] = []
        for idx, root in enumerate(normalized_roots):
            scoped = {
                path.resolve()
                for path in old_sections[idx]
                if path.resolve().is_relative_to(root)
            }
            if idx == selected_section:
                # Keep the rerooted section expanded at its new root so the
                # previous scope remains visible and selectable in-place.
                scoped.add(parent_root)
                scoped.add(selected_scope)
            new_sections.append(scoped)
        self.state.workspace.expanded_by_root = new_sections
        self.state.workspace.expanded = set().union(*new_sections)

        if self.state.workspace.active_root.resolve() == selected_scope:
            self.state.workspace.active_root = parent_root

        self.rebuild_tree_entries(
            preferred_path=selected_target,
            preferred_workspace_root=parent_root,
            preferred_workspace_section=selected_section,
            center_selection=True,
        )
        self.preview_selected_entry(force=True)
        self.schedule_tree_filter_index_warmup()
        self.mark_tree_watch_dirty()
        self.reset_git_watch_context()
        self.refresh_git_status_overlay(force=True)
        self.state.interface.dirty = True

    def reroot_to_selected_target(self) -> None:
        """Replace selected workspace section root with selected target directory."""
        selected_target, target_root = self._selected_target_and_root()
        roots = self._normalize_tree_roots()
        if not roots:
            return
        selected_section = self._selected_workspace_section()
        if selected_section is None:
            active_root = self.state.workspace.active_root.resolve()
            selected_section = next((idx for idx, root in enumerate(roots) if root == active_root), 0)
        selected_section = max(0, min(selected_section, len(roots) - 1))
        selected_scope = roots[selected_section]
        if target_root == selected_scope:
            return

        normalized_roots = list(roots)
        normalized_roots[selected_section] = target_root
        self.state.workspace.roots = normalized_roots

        old_sections = list(self.state.workspace.expanded_by_root)
        while len(old_sections) < len(roots):
            old_sections.append(set())
        new_sections: list[set[Path]] = []
        for idx, root in enumerate(normalized_roots):
            scoped = {
                path.resolve()
                for path in old_sections[idx]
                if path.resolve().is_relative_to(root)
            }
            if idx == selected_section:
                scoped.add(target_root)
            new_sections.append(scoped)
        self.state.workspace.expanded_by_root = new_sections
        self.state.workspace.expanded = set().union(*new_sections)

        if self.state.workspace.active_root.resolve() == selected_scope:
            self.state.workspace.active_root = target_root

        self.rebuild_tree_entries(
            preferred_path=selected_target,
            preferred_workspace_root=target_root,
            preferred_workspace_section=selected_section,
            center_selection=True,
        )
        self.preview_selected_entry(force=True)
        self.schedule_tree_filter_index_warmup()
        self.mark_tree_watch_dirty()
        self.reset_git_watch_context()
        self.refresh_git_status_overlay(force=True)
        self.state.interface.dirty = True

    def add_workspace_root_from_selected_target(self) -> None:
        """Add selected directory as a workspace root without changing root priority."""
        selected_target, target_root = self._selected_target_and_root()
        roots = self._normalize_tree_roots()
        roots.append(target_root)
        self.state.workspace.roots = roots
        sections = list(self.state.workspace.expanded_by_root)
        sections.append({target_root})
        self.state.workspace.expanded_by_root = sections
        self.state.workspace.expanded = set().union(*sections)
        self._normalize_tree_roots()
        new_section = len(self.state.workspace.roots) - 1
        self.rebuild_tree_entries(
            preferred_path=target_root,
            preferred_workspace_root=target_root,
            preferred_workspace_section=new_section,
            center_selection=True,
        )
        self.preview_selected_entry(force=True)
        self.schedule_tree_filter_index_warmup()
        self.mark_tree_watch_dirty()
        self.reset_git_watch_context()
        self.refresh_git_status_overlay(force=True)
        self.state.interface.dirty = True

    def remove_active_workspace_root(self) -> None:
        """Remove selected workspace root (or current-file root) when possible."""
        roots = self._normalize_tree_roots()
        if len(roots) <= 1:
            self.state.interface.status_message = "cannot delete the only root"
            self.state.interface.status_message_until = time.monotonic() + 1.5
            self.state.interface.dirty = True
            return

        selected_section: int | None = None
        if self.state.workspace.entries and 0 <= self.state.workspace.selected < len(self.state.workspace.entries):
            selected_entry = self.state.workspace.entries[self.state.workspace.selected]
            if selected_entry.workspace_section is not None:
                if 0 <= selected_entry.workspace_section < len(roots):
                    selected_section = selected_entry.workspace_section
            elif selected_entry.is_dir and selected_entry.depth == 0:
                candidate = selected_entry.path.resolve()
                selected_section = next((idx for idx, root in enumerate(roots) if root == candidate), None)

        if selected_section is None:
            current_path = self.state.workspace.current_path.resolve()
            selected_section = next(
                (
                    idx
                    for idx, root in sorted(enumerate(roots), key=lambda item: len(item[1].parts), reverse=True)
                    if current_path.is_relative_to(root)
                ),
                None,
            )

        if selected_section is None:
            active_root = self.state.workspace.active_root.resolve()
            selected_section = next((idx for idx, root in enumerate(roots) if root == active_root), len(roots) - 1)

        selected_section = max(0, min(selected_section, len(roots) - 1))
        roots = list(roots)
        sections = list(self.state.workspace.expanded_by_root)
        while len(sections) < len(roots):
            sections.append(set())
        roots.pop(selected_section)
        sections.pop(selected_section)
        self.state.workspace.roots = roots
        self.state.workspace.expanded_by_root = sections
        self.state.workspace.expanded = set().union(*sections)
        # Do not implicitly re-add the previously active root after deletion.
        roots = self._normalize_tree_roots(include_active=False)

        if not any(root == self.state.workspace.active_root.resolve() for root in roots):
            self.state.workspace.active_root = roots[0]

        preferred_path = self.state.workspace.current_path.resolve()
        if not any(preferred_path.is_relative_to(root) for root in roots):
            preferred_path = roots[0]
            self.state.workspace.current_path = preferred_path
        self.rebuild_tree_entries(preferred_path=preferred_path, center_selection=True)
        self.preview_selected_entry(force=True)
        self.schedule_tree_filter_index_warmup()
        self.mark_tree_watch_dirty()
        self.reset_git_watch_context()
        self.refresh_git_status_overlay(force=True)
        self.state.interface.dirty = True

    def toggle_hidden_files(self) -> None:
        """Toggle hidden-file visibility and persist preference to config."""
        self.state.workspace.show_hidden = not self.state.workspace.show_hidden
        save_show_hidden(self.state.workspace.show_hidden)
        selected_path = self.state.workspace.entries[self.state.workspace.selected].path.resolve() if self.state.workspace.entries else self.state.workspace.active_root
        self.rebuild_tree_entries(preferred_path=selected_path)
        self.preview_selected_entry(force=True)
        self.schedule_tree_filter_index_warmup()
        self.mark_tree_watch_dirty()
        self.state.interface.dirty = True

    def toggle_tree_pane(self) -> None:
        """Toggle tree-pane visibility and reflow wrapped source when needed."""
        self.state.layout.browser_visible = not self.state.layout.browser_visible
        if self.state.preview.wrap:
            self.rebuild_screen_lines(columns=self.current_term_columns())
        self.state.interface.dirty = True

    def toggle_wrap_mode(self) -> None:
        """Toggle soft-wrap while preserving approximate top source-line context."""
        top_source_line = source_line_for_display_index(self.state.preview.lines, self.state.preview.scroll)
        self.state.preview.wrap = not self.state.preview.wrap
        if self.state.preview.wrap:
            self.state.preview.horizontal_scroll = 0
        self.rebuild_screen_lines(columns=self.current_term_columns())
        self.state.preview.scroll = first_display_index_for_source_line(self.state.preview.lines, top_source_line)
        self.state.preview.scroll = max(0, min(self.state.preview.scroll, self.state.preview.max_scroll))
        self.state.interface.dirty = True

    def toggle_help_panel(self) -> None:
        """Toggle help panel visibility and keep selected search hit in view."""
        self.state.layout.show_help = not self.state.layout.show_help
        self.rebuild_screen_lines(columns=self.current_term_columns())
        self.ensure_selected_content_hit_visible()
        self.state.interface.dirty = True

    def ensure_selected_content_hit_visible(self) -> None:
        """Adjust source scroll so selected content-hit anchor remains visible."""
        if not (
            self.state.filter.active
            and self.state.filter.mode == "content"
            and self.state.filter.query
            and self.state.workspace.entries
            and 0 <= self.state.workspace.selected < len(self.state.workspace.entries)
        ):
            return

        selected_entry = self.state.workspace.entries[self.state.workspace.selected]
        if selected_entry.kind != "search_hit" or selected_entry.line is None:
            return

        visible_rows = max(1, self.visible_content_rows())
        self.state.preview.max_scroll = max(0, len(self.state.preview.lines) - visible_rows)
        max_line_index = max(0, len(self.state.preview.lines) - 1)
        anchor = max(0, min(selected_entry.line - 1, max_line_index))
        view_end = self.state.preview.scroll + visible_rows - 1
        if self.state.preview.scroll <= anchor <= view_end:
            return

        centered = max(0, anchor - max(1, visible_rows // 3))
        self.state.preview.scroll = max(0, min(centered, self.state.preview.max_scroll))

    def execute_command_palette_action(self, command_id: str) -> bool:
        """Execute command-palette action by id, returning ``True`` to quit."""
        if command_id == "filter_files":
            self.open_tree_filter("files")
            return False
        if command_id == "search_content":
            self.open_tree_filter("content")
            return False
        if command_id == "open_symbols":
            self.open_symbol_picker()
            return False
        if command_id == "history_back":
            if self.jump_back_in_history():
                self.state.interface.dirty = True
            return False
        if command_id == "history_forward":
            if self.jump_forward_in_history():
                self.state.interface.dirty = True
            return False
        if command_id == "toggle_tree":
            self.toggle_tree_pane()
            return False
        if command_id == "toggle_wrap":
            self.toggle_wrap_mode()
            return False
        if command_id == "toggle_hidden":
            self.toggle_hidden_files()
            return False
        if command_id == "toggle_help":
            self.toggle_help_panel()
            return False
        if command_id == "reroot_selected":
            self.reroot_to_selected_target()
            return False
        if command_id == "reroot_parent":
            self.reroot_to_parent()
            return False
        if command_id == "quit":
            return True
        return False

    # picker lifecycle
    def resolve_symbol_target(self) -> Path | None:
        """Resolve file path whose symbols should populate the symbol picker."""
        return self.panel.resolve_symbol_target()

    def open_symbol_picker(self) -> None:
        """Enter symbol-picker mode and populate symbols for current file target."""
        self.panel.open_symbol_picker()

    def open_command_picker(self) -> None:
        """Enter command-palette mode and load command label/id lists."""
        self.panel.open_command_picker()

    def close_picker(self, reset_query: bool = True) -> None:
        """Close picker UI and restore non-picker browser visibility state."""
        self.panel.close_picker(reset_query=reset_query)

    def activate_picker_selection(self) -> bool:
        """Activate current picker row for symbols or command palette actions."""
        return self.panel.activate_picker_selection()
